...
*/
__global__ void threshold(uint16_t *frames, uint64_t n, uint16_t bgt,
                          uint16_t xrt)
{
...
/*
  ...
  frames: the address of the beginning of the frames array
  buf: the address of the beginning of a buffer array (same size as
        frames array) to contain the counted electrons
                        ...
                        uint32_t npixelsperframe)
{
    ...
    // Compute the number of entries in the sparse matrix per frame,
    //  and the number handled by each thread.
    const int thread = threadIdx.x;
    const uint32_t nentries_sparse_frame  =
        (uint32_t) (npixelsperframe/10);
    const uint32_t nentries_sparse_thread =
        (uint32_t) (nentries_sparse_frame/blockDim.x);
    const uint32_t bufoffset =
        f*nentries_sparse_frame + thread*nentries_sparse_thread;
    ...
    for (uint32_t i = thread; i < npixelsperframe; i += stride) {
      ...
      if (row != 0) {
        // Check top sections
        // top
        event = event && pixelValue >
                         frames[frameoffset+topNeighbourRowIndex+column];
        // top left
        event = event &&
                (column == 0 ||
                 pixelValue >
                 frames[frameoffset+topNeighbourRowIndex+leftNeighbourColumn]);
        // top right
        event = event &&
               (column == width - 1 ||
                pixelValue >
                 frames[frameoffset+topNeighbourRowIndex+rightNeighbourColumn]);
      }
      ...
      if (event && row != height - 1) {
        ...
        // bottom
        event = event && pixelValue >
                           frames[frameoffset+bottomNeighbourRowIndex+column];
        // bottom left
        ...
        event =
          ...
          (column == width - 1 ||
           pixelValue >
             frames[frameoffset+bottomNeighbourRowIndex+rightNeighbourColumn]);
      }

      // left
      event = event &&
              (column == 0 || pixelValue >
                  frames[frameoffset+rowIndex+leftNeighbourColumn]);
      // right
      event = event && (column == width - 1 ||
                      pixelValue >
                        frames[frameoffset+rowIndex+rightNeighbourColumn]);
...
void gpu_proc(uint16_t *&h_frames, int *&h_imgNums, int& nfilled,
  ...
  int fsparse, int frame_dim1, int frame_dim2, bool verbose)
{
  ...
  // Lock the buffer mutex for allocation.
  {
    ...
    if(debug) std::cout << "\n[GPU THREAD] Allocating pinned memory..."
                        << std::endl;
    ...
    if(debug) std::cout << "\n[GPU THREAD] Allocating device memory..."
                        << std::endl;
  ...
  for(;;) {
    ...
    // Only proceed when the frames buffer is full, or end the process if
    //  we are done with all frames.
    {
      if(debug) std::cout << "\n[GPU THREAD] Checking for full buffer..."
                          << std::endl;
      std::unique_lock<std::mutex> lk(buffer_mutex);
      ...
      if(buffer_full) {
        ...
        if(debug) std::cout << "\n[GPU THREAD] Buffer full: moving "
                            << nprocess << " frames to GPU" << std::endl;
        bytes_move = nprocess*npixelsperframe*sizeof(uint16_t);

        cudaMemcpyAsync(d_frames,h_frames,bytes_move,
                        cudaMemcpyHostToDevice,stream);
        cudaStreamSynchronize(stream);

        // Set the frames buffer to empty again so that the
        //  reader threads can resume.
        if(debug) std::cout << "\n[GPU THREAD] Setting buffer to empty"
                            << std::endl;
        nfilled = 0;
        buffer_full = false;
        lk.unlock();
    ...
    threshold<<<1, 1024, 0, stream>>>(d_frames, nprocess*npixelsperframe,
                                      backgroundThreshold, xRayThreshold);
    count_sparse<<<nprocess, 1024, 0, stream>>>(d_frames, d_sparse, frame_dim1,
                                                frame_dim2, npixelsperframe);

    // Retrieve the output sparse array.
    bytes_move = nprocess*nsparsepixelsperframe*sizeof(int);
    if(debug) std::cout << "\n[GPU THREAD] Transferring " << bytes_move
                        << " bytes for " << nprocess << " sparse events"
                        << std::endl;
    cudaMemcpyAsync(h_sparse,d_sparse,bytes_move,cudaMemcpyDeviceToHost,stream);
    cudaStreamSynchronize(stream);

    // Process the output sparse array.
    if(debug) std::cout << "\n[GPU THREAD] Processing " << nprocess
                        << " sparse events" << std::endl;

    // Here we need to calculate a modified number of sparse pixels per frame
    //  because we divided the array into 1024 threads. Although the sparse
    //  entries for each frame will be spaced by nsparsepixelsperframe,
    //  there will only be nsparseentriesperthread*1024 of them that are
    //  actually relevant, where nsparsepixelsperthread is the integer division
    //  of nsparsepixelsperframe by 1024. (Essentially there is some
    //  "left over" space in the sparse array because it is not evenly
    ...
    for(uint16_t i = 0; i < nprocess; i++) {
      ...
      // Place the sparse array in the events list (with a mutex lock at the
